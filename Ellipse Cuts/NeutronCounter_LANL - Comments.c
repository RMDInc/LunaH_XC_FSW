...
TCanvas *CPSDN;  
/*
This is a function which processes and plots the energy and PSD from a LANL calibration run, it was created by Erik.
Erik would put this script into a folder with data files and run this script in ROOT. It calls the Processor() function
	on each file listed in the NeutronCounter_LANL() function. It creates pretty plots with the cuts shown on them; both the
	square cuts and the elliptical cuts are shown on the plot. 
The program will loop over each, showing a nice "slideshow" of the data as we moved through the tests.

Take the logic from this function and implement it in the FSW.

@param	basedir		This is a pointer to the directory where the workspace is.
@param	subdir0		A further path, in case we need to descend further to get the files.
@param	subdir1		A further path, in case we need to descend further to get the files.
@param	degree		Angle of rotation for the LANL run
@param	rotaxis		The axis of rotation for the run (X or Z)
@param	source		The radiation source used for the run
@param 	dist		The distance between the source and the Mini-NS
...
void Processor(char *basedir, char *subdir0, char *subdir1,int degree, char *rotaxis,char *source, float dist)
{
	...
	// MinNRG *= 0.8;
/* 
These variables are the temperature correction values which are applied to the run itself, as we recorded one
	temperature for each run in the filename. In the FSW, these will need to be consulted on-the-fly, as the temperature
	will be changing more dynamically in that system. We will probably check the temperature and re-evaluate 
	what the cuts are every few minutes during flight and DAQ.
*/
	...
	//prepare plot creation variables
		numneutrons = 0;
		...
		// PSD = table_val0 + table_val1*temp^2
		...
		//expand the cuts by ~20%
		MaxNRG *= 1.2;
		MinNRG *= 0.8;
		//calculate the semi-minor axis for the ellipse
		brad = (MaxPSD - MinPSD)/2.0;
		//calculate the semi-major axis for the ellipse
		arad = (MaxNRG - MinNRG)/2.0;
		//calculate the center point for the ellipse
		mean_nrg = (MaxNRG + MinNRG)/2.0;
		mean_psd = (MaxPSD + MinPSD)/2.0;

		//Draw the square cuts on the plot
		...
		//draw the ellipse on the plot (xCenter, yCenter, radius r1, radius r2)
		CutEll[mod_idx] = new TEllipse(mean_nrg,mean_psd,arad,brad);
		//adjust drawing parameters (make it pretty)
		...
		//read in the data
		//This script reads in the data file which is produced by Jim's end of run script. That script produces
		// a bunch of "binned" data files which can just be looped over to plot the resulting histograms.
		//There is no inherent data "processing" which goes into producing these plots, it's just reading the 
		// data in from the files and then plotting them with various cuts to see how the cuts do.
		while ( !feof(fI)) {	
			...
			psd /= 99.0;
			psd *= 2.0;
			//write the data to the main histogram
			hPSD[mod_idx]->Fill(nrg,psd,counts);
			
			//This is the logic which is determines if a (energy, psd) value is within the cuts
			//In the specific case of this code, we just check if each bin is within the cuts
			// If the bins resides within the cut area, we add all the counts in that bin to the total
			// else, we discount those counts.
			if ( 
			//see notes p14, notebook 8, (first red book)
			//they rederive the equation that Erik has below
			//+/- requirements because we take a square root and because we want to make sure that the
			//values are within a range 
			/*
			...
			mean_nrg = X center
			mean_psd = Y center
			arad = a, semi-major axis
			brad = b, semi-minor axis
			*/
